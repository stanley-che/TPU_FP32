// tb_attention_score_mod_no_fsm.sv
/*
iverilog -g2012 -Wall -I./src \
  -o ./vvp/tb_att_mod.vvp \
  ./test/tb_attention_score_mod_no_fsm.sv

vvp ./vvp/tb_att_mod.vvp
gtkwave ./vvp/tb_att_mod.vcd
*/
...
module tb_attention_score_mod_no_fsm;
  ...
  // hold start until gemm_busy goes high (handshake-safe)
  @(posedge clk);
  gemm_start <= 1'b1;

  // 等待 busy 真的起來，表示 start 被吃到了
  ...
  // done 後多等 1~2 拍讓 C_valid / SRAM 寫完
...
task automatic print_QK_hex();
  int i,j;
  real f;
  $display("\n==== Q (hex) T x D_LEN=%0d ====", D_LEN);
  for (i=0;i<T;i++) begin
    $write("Q[%0d]: ", i);
  ...
  begin
    ...
    if (e == 0) begin
      if (frac == 0) begin
        ...
        // subnormal: (-1)^s * 2^(1-127) * (frac / 2^23)
        mant = frac / 8388608.0; // 2^23
        val  = mant * (2.0 ** (-126));
      end
    end else if (e == 255) begin
      if (frac == 0) begin
        val = 1.0/0.0; // inf (sign handled below)
      end else begin
        val = 0.0/0.0; // NaN
      end
    end else begin
      // normal: (-1)^s * 2^(e-127) * (1 + frac/2^23)
...
endtask
  ...
  initial begin
    $dumpfile("./vvp/tb_att_mod.vcd");
    $dumpvars(0, tb_attention_score_mod_no_fsm);
    ...
    // ----------------------------
    // Define Q/K in FP32 bits (IEEE754)
    // 1.0=3f800000, 2.0=40000000, 3.0=40400000, ...
    // ----------------------------
    ...
    // [7 8 9]
    // [1 0 1]
    Q_fp[0][0]=32'h3f800000; Q_fp[0][1]=32'h40000000; Q_fp[0][2]=32'h40400000;
    Q_fp[1][0]=32'h40800000; Q_fp[1][1]=32'h40a00000; Q_fp[1][2]=32'h40c00000;
    Q_fp[2][0]=32'h40e00000; Q_fp[2][1]=32'h41000000; Q_fp[2][2]=32'h41100000;
    Q_fp[3][0]=32'h3f800000; Q_fp[3][1]=32'h00000000; Q_fp[3][2]=32'h3f800000;
    ...
    // [1 1 1]
    // [2 0 0]
    K_fp[0][0]=32'h3f800000; K_fp[0][1]=32'h00000000; K_fp[0][2]=32'h40000000;
    K_fp[1][0]=32'h00000000; K_fp[1][1]=32'h3f800000; K_fp[1][2]=32'h40400000;
    K_fp[2][0]=32'h3f800000; K_fp[2][1]=32'h3f800000; K_fp[2][2]=32'h3f800000;
    K_fp[3][0]=32'h40000000; K_fp[3][1]=32'h00000000; K_fp[3][2]=32'h00000000;
    ...
    // wait until C_valid asserts (C SRAM ready)
